...
from meta_rl_tools.nets import PolNetCNP, VNetCNP
...
from common_args import common_args, ppo_args, constant_step_args, random_env_args, sim2real_args, change_batch_size_for_dp, create_env

parser = argparse.ArgumentParser()
parser = common_args(parser)
parser = ppo_args(parser)
parser = constant_step_args(parser)
...
pol_net = PolNetCNP(ob_space, ac_space, h_size=args.h_size, r_size=args.r_size, aggregation=args.aggregation, use_pe=args.use_pe)
...
if isinstance(ac_space, gym.spaces.Box):
    ...
    pol = CategoricalPol(ob_space, ac_space, pol_net, data_parallel=args.data_parallel, parallel_dim=0)
elif isinstance(ac_space, gym.spaces.MultiDiscrete):
    pol = MultiCategoricalPol(ob_space, ac_space, pol_net, data_parallel=args.data_parallel, parallel_dim=0)
else:
...
vf_net = VNetCNP(ob_space, h_size=args.h_size, r_size=args.r_size, aggregation=args.aggregation, use_pe=args.use_pe)
